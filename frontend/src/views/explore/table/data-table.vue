...
<template>
    <div class="space-y-2">
        ...
        <div class="flex items-center justify-between p-3 border-b">
            <div class="flex items-center gap-2">
                ...
                <div class="flex items-center text-xs text-muted-foreground" v-if="stats?.rows_read !== undefined">
                    ...
                    <span class="text-xs">
                        ...
                        <template v-if="stats?.execution_time_ms !== undefined">
                            in {{ (stats.execution_time_ms / 1000).toFixed(3) }}s
                        </template>
                    </span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="relative w-64">
                    ...
                    <Input placeholder="Search in all columns..." v-model="globalFilter" class="pl-8 h-8 text-sm" />
                </div>
            </div>
        </div>
        ...
        <div class="rounded-md border-0">
            <div v-if="table.getRowModel().rows?.length" class="overflow-auto max-h-[calc(100vh-280px)]">
                            ...
                            <tr v-if="row.getIsExpanded()">
                                <td :colspan="row.getVisibleCells().length" class="p-0">
                                    <div class="p-4 bg-muted/30 border-t border-b border-muted">
                                        <JsonViewer :value="row.original" :expanded="false" class="text-sm" />
                                    </div>
                                </td>
                            </tr>
                        </template>
            ...
            <div v-else class="h-[calc(100vh-350px)]">
                <EmptyState />
            </div>
        </div>

        <!-- Bottom pagination controls -->
        <div v-if="table.getRowModel().rows?.length > 0" class="flex items-center justify-end border-t pt-2 px-2">
